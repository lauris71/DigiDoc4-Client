...
CDoc2Reader::getFMK(const libcdoc::CKey &key, const std::vector<uint8_t>& secret)
{
	...
	if (key.isSymmetric()) {
		...
		if (pki.pk_type == libcdoc::CKey::PKType::RSA) {
			kek = crypto->decryptRSA(key_material, true);
		} else {
			...
			//QByteArray info = KEK + cdoc20::header::EnumNameFMKEncryptionMethod(cdoc20::header::FMKEncryptionMethod::XOR) + bytes + key_material;
...
bool
CDoc2::save(std::ostream& ofs, const std::vector<File>& files, const std::vector<std::shared_ptr<libcdoc::CKey>>& keys, const std::vector<uint8_t>& secret)
{
	setLastError({});
	std::vector<uint8_t> fmk = libcdoc::Crypto::extract(libcdoc::Crypto::random(KEY_LEN), std::vector<uint8_t>(SALT.cbegin(), SALT.cend()));
	std::vector<uint8_t> cek = libcdoc::Crypto::expand(fmk, std::vector<uint8_t>(CEK.cbegin(), CEK.cend()));
	std::vector<uint8_t> hhk = libcdoc::Crypto::expand(fmk, std::vector<uint8_t>(HMAC.cbegin(), HMAC.cend()));
#ifndef NDEBUG
	std::cerr << "fmk: " << libcdoc::Crypto::toHex(fmk) << std::endl;
	...
	for(std::shared_ptr<libcdoc::CKey> key: keys) {
		if (key->isPKI()) {
			...
			if(pki.pk_type == libcdoc::CKey::PKType::RSA) {
				...
				std::vector<uint8_t> kekPm = libcdoc::Crypto::extract(sharedSecret, std::vector<uint8_t>(KEKPREMASTER.cbegin(), KEKPREMASTER.cend()));
				std::string info_str = KEK +
						cdoc20::header::EnumNameFMKEncryptionMethod(cdoc20::header::FMKEncryptionMethod::XOR) +
						std::string(pki.rcpt_key.cbegin(), pki.rcpt_key.cend()) +
	...
	std::vector<uint8_t> nonce = libcdoc::Crypto::random(NONCE_LEN);
#ifndef NDEBUG
	...
	if(!libcdoc::TAR::save(zofs, files)) {
		return false;
	}
	zofs.close();
	if(!enc.result()) {
