...
QHash<QString, QString> CKey::fromKeyLabel() const
{
    ...
    if (!CKeyCD2::isCDoc2Key(*this)) return result;
    CKeyCD2 *cd2 = (CKeyCD2 *) this;
    QString rcpt(cd2->label);
...
{
    if (CKeyCD2::isCDoc2Key(*this)) {
        CKeyCD2 *cd2 = (CKeyCD2 *) this;
        return cd2->label;
    }
    if (!CKeyCD1::isCDoc1Key(*this)) return {};
    CKeyCD1 *cd1 = (CKeyCD1 *) this;
    if(cd1->cert.isNull())
        return cd1->recipient;
...
{
	//std::shared_ptr<CKey> ckey = d->cdoc->getDecryptionKey(cert);
	//return (ckey != nullptr) && !ckey->key.isEmpty();
