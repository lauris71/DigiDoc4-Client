...
{
	return libcdoc::Crypto::concatKDF(digest, keySize, derive(publicKey), algorithmID, partyUInfo, partyVInfo);
}
...
std::vector<uchar> PKCS12Token::derive(const std::vector<uchar> &publicKey) const
{
	...
	return libcdoc::Crypto::deriveSharedSecret(d->pkey.get(), peerPKey.get());
}
